...
//		美人百宝袋新增开启机制：
//		1、当日充值金额>999RMB的玩家开启美人百宝袋时，排除8888元宝奖励。
//		2、已抽取到1次8888元宝的玩家，给该玩家记录一次获奖标识，当玩家拥有该标识时，不能抽取8888元宝。15天后该标识消失。
...
//					&&player.getGold().getIsinner()==0
				...
				//MessageUtil.notify_All_player(Notifys.SROLL, "{1}玩家开启江山百宝袋获得8888元宝大奖",player.getName());
				//MessageUtil.notify_All_Activity(Notifys.CHAT_SYSTEM, 21, "{1}玩家开启江山百宝袋获得8888元宝大奖",player.getName());
				player.getActivitiesReward().put("mrbbdlasttime", System.currentTimeMillis()+"");
				MessageUtil.notify_player(player, Notifys.CHAT_BULL, ResManager.getInstance().getString("恭喜您开启江山百宝袋获得8888元宝大奖"));
				ParseUtil parseUtil = new ParseUtil();
				parseUtil.setValue(String.format(ResManager.getInstance().getString("玩家【%s】开启【江山百宝袋】获得8888元宝大奖，真是羡煞旁人！{@}"), player.getName()), new ParseUtil.VipParm(VipManager.getInstance().getPlayerVipId(player),GuideType.BAIBAODAI.getValue()));
